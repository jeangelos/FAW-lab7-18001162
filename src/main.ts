...
import { searchCountries, ApiError, getCountriesByRegion, getAllCountries } from './services/countryApi';
...
let selectedRegion: 'Africa' | 'Americas' | 'Asia' | 'Europe' | 'Oceania' | 'all' = 'all';
...
async function handleSearch(): Promise<void> {
  ...
  if (query.length === 0) {
   ...
   // Si búsqueda vacía y región "all", volvemos al estado inicial
  if (query.length === 0 && selectedRegion === "all") {
    render({ status: 'idle' });
    lastSearchQuery = '';
    return;
  }
